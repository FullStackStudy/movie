...
public class ReservationController {
    ...
    //예약페이지 in
    @GetMapping
    public String reserveSeats(@AuthenticationPrincipal UserDetails userDetails, Model model,HttpSession session){
        String member_id = userDetails.getUsername();
        System.out.println("member_id: "+ member_id);
        model.addAttribute("member_id", member_id);

        Long fixedRoomId= 1L; //영화관 1관으로 고정 나중에 수정
        List<Seat> seatList = seatRepository.findByRoomId(fixedRoomId);
        List<SeatDto> seatDtos = SeatDto.ofList(seatList);

        //지금은 무비로 나주엥 스케쥴로 바꿔야함
        //List<ReservedSeatDto> reservedSeatDtos = reservedSeatRepository.findBy()
    ...
    public @ResponseBody ResponseEntity<String> reserveSeats(@RequestBody ReservationDto reservationDto, HttpSession session){
        ...
        if(!result.getFailList().isEmpty()){
            for(String seatId : result.getSuccessList()){ //성공해서 레디스에 저장한 걸
                reservationService.releaseSeat(reservationDto.getMovie_id(),seatId, reservationDto.getMember_id()); //redis 삭제
            }
            ...
            System.out.println("reDto member_id" + reservationDto.getMember_id());
    ...
    //결제
    @PostMapping("/pay")
    public String payReservation(@ModelAttribute ReservationDto reservationDto, Model model, @AuthenticationPrincipal UserDetails userDetails) {
        System.out.println(reservationDto.getMovie_id() + "영화정보");
        System.out.println(reservationDto.getSeat_id() + "좌석정보");
        System.out.println(reservationDto.getMember_id() + "좌석정보");
        System.out.println(reservationDto.getPay_method() + " 결제정보");
        ...
        if (!failPay) { //결제 취소하면, 결제 화면 나가면 그냥 결제 화면 보여주는거잖아? //이건 아예 예약확정 전에 예약취소하는거지 -> 수정하기
            for (String seatId : reservationDto.getSeat_id()) { //좌석
                reservationService.releaseSeat(reservationDto.getMovie_id(), seatId, reservationDto.getMember_id());
                model.addAttribute("errorMessage", "결제 실패");
            }
            return "redirect:/reservation/pay";
        } else { //결제 성공
            boolean success = reservationService.saveReservation(userDetails ,reservationDto);
    ...
    public @ResponseBody ResponseEntity<String> backSeat(HttpSession session,Model model, @RequestParam("type") String type){
        ...
        if(type.equals("back")){
            ReservationDto reservationDto = (ReservationDto) session.getAttribute("reservedData");
            System.out.println("세션 정보"+ reservationDto.getMovie_id());
