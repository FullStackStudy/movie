...
public class KakaoPayService {
    ...
    public KakaoPayCancelResponseDto kakaoPayCancel(KakaoPayCancelRequestDto requestDto) {
        // 카카오페이 환불 요청 양식
        Map<String, Object> parameters = new HashMap<>();
        parameters.put("cid", cid);
        parameters.put("tid", requestDto.getTid());
        parameters.put("cancel_amount", requestDto.getCancel_amount());
        parameters.put("cancel_tax_free_amount", requestDto.getCancel_tax_free_amount());
        parameters.put("cancel_reason", requestDto.getCancel_reason());

        // 파라미터, 헤더
        HttpEntity<Map<String, Object>> requestEntity = new HttpEntity<>(parameters, this.getHeaders());

        try {
            KakaoPayCancelResponseDto responseDto = paymentRestTemplate.postForObject(
                    "https://open-api.kakaopay.com/online/v1/payment/cancel",
                    requestEntity, KakaoPayCancelResponseDto.class);
            log.info("카카오페이 환불 응답: {}", responseDto);
            return responseDto;
        } catch (Exception e) {
            log.error("카카오페이 환불 요청 실패", e);
            throw new RuntimeException("카카오페이 환불 요청에 실패했습니다.", e);
        }
    }
